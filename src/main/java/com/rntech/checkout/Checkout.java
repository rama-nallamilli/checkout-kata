...
public class Checkout {
  ...
  private Integer calculatePrice(ItemSku sku, Integer quantity, Map<ItemSku, ItemPrice> rules) {
    final ItemPrice pricing = Optional.ofNullable(rules.get(sku)).orElseThrow(() -> new SkuNotFoundException(sku));
    final PricingRule rule = pricing.getPricingRule().orElse(defaultPricingRule);
